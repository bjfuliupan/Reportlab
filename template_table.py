...
def test_pdf():
    # 读取PDF模板文件
    tpl = os.path.join(os.getcwd(), "templates", "template_tables.xml")
    ...
    try:
        ...
        # 设置标题显示内容
        PDFTemplate.set_paragraph_data(pages, 0, "title", "刘攀的第一个PDF测试文档2")

        # 设置描述说明内容
        description = ("<b>A pa<font color=red>r</font>a<i>graph</i></b><super><font color=yellow>1</font></super>"
                       "<font color=red size=15>刘攀</font>的第一个PDF测试文档。第1行我们导入Paragraph和SimpleDocTemplate类。"
        ...
        PDFTemplate.set_table_data(pages, 0, "table", data)
        ...
        PDFTemplate.set_pie_chart_data(pages, 2, "pie_chart", [170, 165, 167, 172, 176, 180, 160, 166],
                                       ...
                                       main_title="aaaaaaaaaaaaaaaaaaa")
