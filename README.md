...
```
pdf													
	file_name												生成的PDF文件的路径
	author												PDF文件作者。默认为空
	title												PDF文件标题。默认为空
	page_size												每页的大小，格式：[width, height]
	...
	pages												
		page0											
			<font color="red">rect</font>										当前页的有效区域。格式：[x, y, width, height]
			...
			items										
				[item_name1]									
					type								text。该item为文本段
					rect								该item的绘画区域。格式：[x, y, width, height]。该坐标相对于page的rect而言
					content								该文本段的内容
				...
				[item_name3]									
					type								line_chart。该item为折线图
					rect								该item的绘画区域。格式：[x, y, width, height]。该坐标相对于page的rect而言
					data								折线图的数据。格式：[(数据1), (数据2), …]
					main_title								主标题的内容
				...
				[item_name4]									
					type								bar_chart。该item为柱状图
					rect								该item的绘画区域。格式：[x, y, width, height]。该坐标相对于page的rect而言
					data								柱状图的数据。格式：[(数据1), (数据2), …]
					bar_style								柱状图的style。取值：vertical、horizontal。horizontal是水平柱状图，vertical是垂直柱状图
					main_title								主标题的内容
					main_title_font_name								主标题的字体名称。默认为SimSun
					main_title_font_color								主标题的字体颜色。格式：Color(r, g, b, a)，r、g、b和a的取值范围：[0, 1]。默认为黑色Color(0, 0, 0, 1)
					main_title_font_size								主标题的字体大小。默认为10
				...
				[item_name5]									
					type								pie_chart。该item为饼图
					rect								该item的绘画区域。格式：[x, y, width, height]。该坐标相对于page的rect而言
					data								饼图的数据。格式：[数值1, 数值2, …]
					main_title								主标题的内容
					main_title_font_name								主标题的字体名称。默认为SimSun
					main_title_font_color								主标题的字体颜色。格式：Color(r, g, b, a)，r、g、b和a的取值范围：[0, 1]。默认为黑色Color(0, 0, 0, 1)
					main_title_font_size								主标题的字体大小。默认为10
...
try:
    ...
    pdf.set_item_data(0, "line_chart1", data=data)
    pdf.set_item_data(0, "bar_chart1", data=data)
    pdf.set_item_data(0, "pie_chart1", data=data)
    # ......
