...
def log_format_to_detail(log_format: str) -> str:
    # 将日志类型转化为中文名称
    return constant.LogFormat.FORMAT_MAPPING.get(log_format, "未定义日志")


def datetime_to_category_time(target_time: datetime) -> str:
    # 将 datetime 格式时间转换为 折线图横坐标时间格式(%Y-%m-%d)
    return target_time.strftime(constant.LogFormat.CATAEGORY_TIME_FORMAT)


def adjust_es_log_time(log_time: str) -> datetime:
    # 从ES获取的日志格式形如： 2019-07-03T16:00:00.000Z， 且时间滞后8小时，需要重新格式化和校准
    return datetime.strptime(log_time, constant.LogFormat.ES_LOG_TIME_FORMAT)
